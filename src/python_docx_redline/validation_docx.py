...
class DOCXSchemaValidator(BaseSchemaValidator):
    ...
    def validate(self):
        ...
        # Test 1: Encoding declarations (CRITICAL - must be UTF-8 or UTF-16)
        all_valid = True
        if not self.validate_encoding_declarations():
            all_valid = False

        # Test 2: Namespace declarations
        ...
        # Test 8: Deletion validation
        if not self.validate_deletions():
            all_valid = False

        # Test 9: Insertion validation
        if not self.validate_insertions():
            all_valid = False

        # Test 10: Relationship ID reference validation

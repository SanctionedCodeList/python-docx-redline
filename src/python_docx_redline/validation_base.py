...
class BaseSchemaValidator:
    ...
    def __init__(self, unpacked_dir, original_file, verbose=False):
        ...
        # Set schemas directory (in same package as validation modules)
        ...
        if not self.xml_files:
            logger.warning("No XML files found in %s", self.unpacked_dir)

    def validate(self):
        """Run all validation checks and return True if all pass."""
        raise NotImplementedError("Subclasses must implement the validate method")

    def validate_encoding_declarations(self):
        """
        Validate that all XML files use UTF-8 or UTF-16 encoding.

        OOXML specification requires that all XML files use either UTF-8 or UTF-16
        encoding. Other encodings like ASCII, even though technically compatible,
        will cause the document to be rejected by Microsoft Word and other consumers.
        """
        import re
        ...
        for xml_file in self.xml_files:
            try:
                # Read the file as bytes to check encoding declaration
                ...
                # Pattern matches: encoding="..." or encoding='...'
                pattern = r'encoding=["\'](.*?)["\']'
                match = re.search(pattern, text[:200])

                if match:
                    encoding = match.group(1).upper()
                    # OOXML only supports UTF-8 and UTF-16
                    valid_encodings = ["UTF-8", "UTF-16", "UTF-16LE", "UTF-16BE"]
                    if encoding not in valid_encodings:
                        rel_path = xml_file.relative_to(self.unpacked_dir)
                        errors.append(
                            f"  {rel_path}: Invalid encoding '{encoding}' "
                            f"(OOXML requires UTF-8 or UTF-16)"
                        )

            except Exception as e:
                rel_path = xml_file.relative_to(self.unpacked_dir)
                errors.append(f"  {rel_path}: Error checking encoding: {e}")

        if errors:
            logger.error(
                "FAILED - Found %d encoding declaration violations:\n%s\n"
                "CRITICAL: Files with non-UTF-8/UTF-16 encodings will cause the document "
                "to be rejected by Microsoft Word and other OOXML consumers.",
                len(errors),
                "\n".join(errors),
            )
            return False
            ...
            if self.verbose:
                logger.info("PASSED - All files use valid OOXML encodings (UTF-8 or UTF-16)")
            return True
    ...
    def validate_xml(self):
        ...
        if errors:
            logger.error("FAILED - Found %d XML violations:\n%s", len(errors), "\n".join(errors))
            return False
        else:
            if self.verbose:
    ...
    def validate_namespaces(self):
        ...
        if errors:
            logger.error("FAILED - %d namespace issues:\n%s", len(errors), "\n".join(errors))
            return False
        if self.verbose:
    ...
    def validate_unique_ids(self):
        ...
        if errors:
            logger.error(
                "FAILED - Found %d ID uniqueness violations:\n%s", len(errors), "\n".join(errors)
            )
            return False
        else:
            if self.verbose:
    ...
    def validate_file_references(self):
        ...
        Note: Uses case-insensitive path matching to handle Microsoft Word's inconsistent
        casing (e.g., customXml vs customXML). While OOXML ZIP is technically case-sensitive,
        Word itself creates documents with inconsistent casing, so we must tolerate this.
        """
        errors = []
        ...
        # Build case-insensitive lookup for all files
        # Maps lowercase relative path -> actual file path
        case_insensitive_lookup = {}
        for file_path in all_files:
            rel_path = file_path.relative_to(self.unpacked_dir.resolve())
            lower_path = str(rel_path).lower()
            case_insensitive_lookup[lower_path] = file_path

        # Track all files that are referenced by any .rels file
        all_referenced_files = set()

        if self.verbose:
            logger.info("Found %d .rels files and %d target files", len(rels_files), len(all_files))
            ...
            try:
                ...
                for rel in rels_root.findall(
                    ...
                    if target and not target.startswith(("http", "mailto:")):  # Skip external URLs
                        ...
                        # Use case-insensitive matching to handle Word's inconsistent casing
                        # (e.g., customXml vs customXML in the same document)
                        try:
                            target_path = target_path.resolve()
                            # Always use case-insensitive lookup to get the correctly-cased
                            # actual file path. This handles:
                            # 1. Case-sensitive filesystems (Linux) where path doesn't exist
                            # 2. Case-insensitive filesystems (macOS/Windows) where path
                            #    exists but with wrong case
                            rel_target = str(target_path.relative_to(self.unpacked_dir.resolve()))
                            lower_target = rel_target.lower()
                            if lower_target in case_insensitive_lookup:
                                actual_file = case_insensitive_lookup[lower_target]
                                referenced_files.add(actual_file)
                                all_referenced_files.add(actual_file)
                                # Log case mismatches in verbose mode
                                if self.verbose and actual_file != target_path:
                                    logger.debug(
                                        "Case mismatch - '%s' resolved to '%s'",
                                        target,
                                        actual_file.relative_to(self.unpacked_dir),
                                    )
                            elif target_path.exists() and target_path.is_file():
                                # File exists and matches case exactly (not in lookup means
                                # it could be a new file added after the lookup was built)
        ...
        if errors:
            logger.error(
                "FAILED - Found %d relationship validation errors:\n%s\n"
    ...
    def validate_all_relationship_ids(self):
        ...
        if errors:
            logger.error(
                "FAILED - Found %d relationship ID reference errors:\n%s\n"
    ...
    def validate_content_types(self):
        ...
        if errors:
            logger.error(
                "FAILED - Found %d content type declaration errors:\n%s",
                len(errors),
                "\n".join(errors),
            )
            return False
    ...
    def validate_file_against_xsd(self, xml_file, verbose=False):
        ...
        if new_errors:
            ...
            if verbose:
                logger.info("PASSED - No new errors (original had %d errors)", len(current_errors))
            return True, set()
    ...
    def validate_against_xsd(self):
        ...
        if self.verbose:
            ...
            if original_error_count:
                summary += f"\n  - With original errors (ignored): {original_error_count}"
            summary += f"\n  - With NEW errors: {new_err_count if new_errors else 0}"
            logger.info(summary)

        if new_errors:
            logger.error("FAILED - Found NEW validation errors:\n%s", "\n".join(new_errors))
            return False
        else:
            if self.verbose:
                logger.info("PASSED - No new XSD validation errors introduced")
            return True
